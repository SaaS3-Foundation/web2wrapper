import { Controller, Response, Get, Query } from '@nestjs/common';
    ...
    let ok = teams.every((team) => {
      ...
      for (let i = matday_len - 1; i >= 0; i--) {
        ...
        for (let j = matches_len - 1; j >= 0; j--) {
          ...
          console.log(match);
          if (
            this.same(match.homeParticipant.participantName, team[0]) &&
            this.same(match.awayParticipant.participantName, team[1])
          ) {
            return true;
          }
        }
  ...
  async played(@Query() q: any): Promise<number> {
    ...
    for (let i = matday_len - 1; i >= 0; i--) {
      ...
      for (let j = matches_len - 1; j >= 0; j--) {
        ...
        if (
          this.same(match.homeParticipant.participantName, q.home) &&
          this.same(match.awayParticipant.participantName, q.guest)
        ) {
          found = true;
          ...
          if (match_result == 2) {
            // check extra time
            console.log('check extra time');
            if (
              match.extraTime !== undefined &&
              match.extraTime.is_extra === '1'
            ) {
              if (match.extraTime.home_score > match.extraTime.away_score) {
                match_result = 1;
              } else if (
                match.extraTime.home_score < match.extraTime.away_score
              ) {
                match_result = 0;
              } else {
                match_result = 2;
          ...
          if (match_result == 2) {
            console.log('check Penalty Shootout');
            if (match.stages !== undefined && match.stages.length > 0) {
              let st = match.stages[0];
              if (st.home_score != null && st.away_score != null) {
                if (st.home_score > st.away_score) {
                  match_result = 1;
                } else if (st.home_score < st.away_score) {
                  match_result = 0;
                }
              }
            }
